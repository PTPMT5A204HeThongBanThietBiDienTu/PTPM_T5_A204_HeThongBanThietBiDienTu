...
const Payment = (props) => {
    const { name, setCartCookie } = props
    ...
    const userGuess = "37708cfa-365d-48d5-893b-cacf8b87f6cc"
    const handleDeleteCartCookie = () => {
        setCartCookie([]);
    ...
    const handleSubmit = async (event) => {
        ...
        if (selectedItem) {
            if (name !== '') {
                ...
                axios.post(`http://localhost:7777/api/v1/bill/createCookie?userId=${userGuess}`, dataOrderInfo)
                    .then(res => {
                        if (res && res.data.success === true) {
                            const billId = res.data.data.id;
                            handleDeleteCartCookie();
                            swal({
                                title: "Đặt hàng thành công!",
                                icon: "success",
                            }).then(() => {
    ...
    return (
        dataOrderInfo !== undefined && total !== undefined ? (
